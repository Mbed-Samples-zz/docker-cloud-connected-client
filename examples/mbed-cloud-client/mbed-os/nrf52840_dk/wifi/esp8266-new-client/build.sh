...
mbed compile -m ${TARGET_NAME} -t ${MBED_OS_COMPILER} --profile ${BOOTLOADER_BUILD_PROFILE} --build BUILD/${TARGET_NAME}/${MBED_OS_COMPILER} >> mbed-compile-bootloader.log
...
echo "---> Set the storage type"
jq '."target_overrides"."NRF52840_DK"."target.components_add" = ["SPIF"]' mbed_app.json | sponge mbed_app.json
...
mbed compile -m ${TARGET_NAME} -t ${MBED_OS_COMPILER} --profile ${APP_BUILD_PROFILE} --build BUILD/${TARGET_NAME}/${MBED_OS_COMPILER} >> ${EPOCH_TIME}-mbed-compile-client.log
...
echo "---> Copy builds to ~/Share/${EPOCH_TIME}-Source"
cp -R ~/Source ~/Share/${EPOCH_TIME}-Source
